...
testGroup "HTMLParser", ->
  eachFixture (name, {html, serializedHTML, document}) ->
    test name, ->
      parsedDocument = HTMLParser.parse(html).getDocument()
      assert.documentHTMLEqual parsedDocument.copyUsingObjectsFromDocument(document), html

  eachFixture (name, {html, serializedHTML, document}) ->
    if serializedHTML?
      test "#{name} (serialized)", ->
        parsedDocument = HTMLParser.parse(serializedHTML).getDocument()
        assert.documentHTMLEqual parsedDocument.copyUsingObjectsFromDocument(document), html
  ...
  testGroup "nested line breaks", ->
    ...
    for html, expectedHTML of cases
      do (html, expectedHTML) ->
        test html, -> assert.documentHTMLEqual HTMLParser.parse(html).getDocument(), expectedHTML
  ...
  test "parses absolute image URLs", ->
    ...
    finalHTML = getHTML(HTMLParser.parse(html).getDocument())
  ...
  test "parses attachment caption from large html string", (done) ->
    ...
    for n in [1..3]
      attachmentPiece = HTMLParser.parse(html).getDocument().getAttachmentPieces()[0]
